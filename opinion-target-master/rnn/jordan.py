...
class model(object):
    ...
    def __init__(self, nh, nc, ne, de, cs, em, init):
        ...
        def recurrence(x_t, s_tm1):
            h_t = T.nnet.sigmoid(T.dot(x_t, self.Wx) + T.dot(s_tm1, self.Ws) + self.bh)
            s_t = T.nnet.softmax(T.dot(h_t, self.W) + self.b)[0]
            return [h_t, s_t]

        [h, s], _ = theano.scan(fn=recurrence, sequences = x, outputs_info=[None, self.s0], n_steps = x.shape[0])
